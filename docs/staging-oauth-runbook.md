...
applying any migration. The Better Auth `1.6.23` comparison introduces no
database migration for the enabled protocol.
...
Then set `ENABLE_OAUTH_TOKEN_ISSUANCE=true` for the guarded verification window.
...
E2E_CALLBACK_ORIGIN=http://127.0.0.1:5010
SSO_ISSUER=https://staging-api.example.com/api/auth
...
Do not commit credentials or reuse personal/production owner accounts. The
loopback callback stays on the test runner and is provisioned as the run-owned
client's exact callback and origin.
...
Admin must pass application lifecycle and OAuth/JWKS verification. User must
pass OAuth/JWKS verification while application management remains skipped and
admin routes remain forbidden. Confirm cleanup removed the run prefix and
released the actor lock after each run.
...
1. Set `ENABLE_OAUTH_TOKEN_ISSUANCE=false` and redeploy/restart the API.
2. Confirm the SSO-owned token endpoint returns `404`.
