...
import { deleteCachedEffectivePermissions } from "./effective";
import { RBAC_CATALOG_VERSION_KEY } from "../keys";

export async function invalidateUser(userId: string) {
  await deleteCachedEffectivePermissions(userId);
}
...
export async function invalidateRole(roleId: string, userIds: string[]) {
