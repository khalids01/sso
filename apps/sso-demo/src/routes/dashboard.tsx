  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-5 pt-10 md:px-8 md:pt-16">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div><Badge className="mb-4 border-emerald-400/30 bg-emerald-400/10 text-emerald-300"><Check className="mr-1.5 size-3.5" />Verified session</Badge><h1 className="text-4xl font-semibold tracking-[-0.04em]">Welcome, {session.user.name}</h1><p className="mt-3 text-muted-foreground">The library exchanged the authorization code, verified both tokens, and created this application session.</p></div>
      </div>

      <div className="mt-10 grid gap-5 md:grid-cols-3">
        <Metric icon={Fingerprint} label="Pairwise subject" value={`${session.user.id.slice(0, 14)}…`} />
        <Metric icon={KeyRound} label="Scope" value={session.user.scope} />
        <Metric icon={ShieldCheck} label="Authorization version" value={String(session.user.authorizationVersion)} />
      </div>
      ...
      <Card className="mt-5 bg-card/80 backdrop-blur-xl">
        ...
        <CardContent className="grid gap-x-10 gap-y-5 pt-6 md:grid-cols-2">
          ...
          <Claim label="Expires" value={new Date(session.expiresAt).toLocaleString()} />
        </CardContent>
      </Card>
    </main>
  );
